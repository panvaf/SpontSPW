# used to run already trained network PB
...
tauI = 4*ms      # Decay constant of GABA-type conductance for SOM cells
...
e_to_e.connect(i=PBcon['PPi'],j=PBcon['PPj'])
i_to_e.connect(i=PBcon['PBi'],j=PBcon['PBj'])
e_to_i.connect(i=PBcon['BPi'],j=PBcon['BPj'])
i_to_i.connect(i=PBcon['BBi'],j=PBcon['BBj'])

# initialize

i_to_e.W = PBcon['W']
neurons.V = np.random.randint(Vr/mV,Vt/mV,Ne+Ni)*mV  # random initial conditions 
#neurons.V = PBcon['V']*volt
#neurons.gE = PBcon['gE']*siemens
#neurons.gI = PBcon['gI']*siemens
